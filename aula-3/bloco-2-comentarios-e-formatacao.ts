// =============================================================================
// BLOCO 2 — Comentários sob suspeita e formatação automatizada
// =============================================================================
// Prática (~10 min):
//   1) Rode o formatador (Prettier/Biome) e veja a indentação se ajeitar.
//   2) Apague comentários redundantes ou troque-os por nomes melhores.
//   3) Mantenha apenas comentários legítimos (aviso, "por quê", TODO).
// =============================================================================

export class Usuario {
    public id: string;
    public nomeDoUsuario: string;
    public idade: number;

    constructor(id: string, nomeDoUsuario: string, idade: number){
        this.id = id;
        this.nomeDoUsuario = nomeDoUsuario;
        this.idade = idade;
    }

    public ehMaiorIdade(): boolean {
        if (this.idade >= 18) {
            return true;
        } else {
            return false;
        }
    }
}

export function calcularSoma(numeroUm: number, numeroDois: number){
    return numeroUm + numeroDois;
}

console.log(new Usuario("u1", "Ana", 20).ehMaiorIdade());
console.log(calcularSoma(2, 3));
